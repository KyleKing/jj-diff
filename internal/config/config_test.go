...
func TestDefaultConfig(t *testing.T) {
	...
	if cfg.ViewMode != config.ViewModeUnified {
...
func TestLoadConfigFromEnv(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			checkFn:  func(c config.Config) bool { return c.ViewMode == config.ViewModeSideBySide },
			expected: true,
		},
		{
			name:     "unified mode explicit",
			envVars:  map[string]string{"JJ_DIFF_VIEW_MODE": "unified"},
			checkFn:  func(c config.Config) bool { return c.ViewMode == config.ViewModeUnified },
			expected: true,
		},
		{
			name:     "show whitespace true",
			envVars:  map[string]string{"JJ_DIFF_SHOW_WHITESPACE": "true"},
			checkFn:  func(c config.Config) bool { return c.ShowWhitespace },
			expected: true,
		},
		{
		...
		},
		{
			...
			checkFn:  func(c config.Config) bool { return c.ShowLineNumbers },
			expected: false,
		},
		{
			name:     "tab width 8",
			envVars:  map[string]string{"JJ_DIFF_TAB_WIDTH": "8"},
			checkFn:  func(c config.Config) bool { return c.TabWidth == 8 },
			expected: true,
		},
		{
			name:     "tab width invalid stays default",
			envVars:  map[string]string{"JJ_DIFF_TAB_WIDTH": "invalid"},
			checkFn:  func(c config.Config) bool { return c.TabWidth == 4 },
			expected: true,
		},
		{
		...
		},
		{
			...
			checkFn:  func(c config.Config) bool { return c.WordLevelDiff },
			expected: true,
		},
	}
...
func TestParseBool(t *testing.T) {
	...
	for _, tt := range tests {
		if got := config.ParseBool(tt.input); got != tt.expected {
